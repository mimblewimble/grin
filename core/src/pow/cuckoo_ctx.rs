...
use pow::common::{self, EdgeType, Edge};
	...
	T: EdgeType,
{
	...
	fn update_graph(&mut self, mut nu: usize, us: &[T], mut nv: usize, vs: &[T]) -> Result<(), Error> {
		if nu < nv {
			while nu != 0 {
				nu -= 1;
	...
	fn find_sol(&mut self, mut nu: usize, us: &[T], mut nv: usize, vs: &[T]) -> Result<Vec<T>, Error> {
		if us[nu] == vs[nv] {
	...
	pub fn find_cycles(&mut self) -> Result<Vec<Proof>, Error> {
		...
		for nonce in 0..self.easiness.to_u64().ok_or(ErrorKind::IntegerCast)? as usize {
			...
			let nu = self.path(u, &mut us)?.to_u64().ok_or(ErrorKind::IntegerCast)? as usize;
			let nv = self.path(v, &mut vs)?.to_u64().ok_or(ErrorKind::IntegerCast)? as usize;
...
mod test {
	...
	T: EdgeType,
	{
		...
		cuckoo_ctx.set_header_nonce([49].to_vec(), None)?;
		assert!(
			cuckoo_ctx.verify(&Proof::new(V1.to_vec().clone())).is_ok()
		);
		let mut cuckoo_ctx = CuckooContext::<T>::new(20, 42, 70, 10)?;
		cuckoo_ctx.set_header_nonce([50].to_vec(), None)?;
		assert!(
			cuckoo_ctx.verify(&Proof::new(V2.to_vec().clone())).is_ok()
		);
		cuckoo_ctx.set_header_nonce([51].to_vec(), None)?;
		assert!(
			cuckoo_ctx.verify(&Proof::new(V3.to_vec().clone())).is_ok()
		);
		Ok(())
	}
	...
	T: EdgeType
	{
		...
		// edge checks
		assert!(
			!cuckoo_ctx.verify(&Proof::new(vec![0; 42])).is_ok()
		);
		assert!(
			!cuckoo_ctx.verify(&Proof::new(vec![0xffff; 42])).is_ok()
		);
		...
		assert!(
			!cuckoo_ctx.verify(&Proof::new(V4.to_vec().clone())).is_ok()
		);
		Ok(())
	}
