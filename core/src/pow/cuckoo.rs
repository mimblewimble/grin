...
impl Cuckoo {
	...
	pub fn from_hash(header_hash: &[u8], sizeshift: u8) -> Cuckoo {
		println!("header:{:?}", header_hash);
		let size = 1 << sizeshift;
		println!("Edge mask: {}", (1 << sizeshift) / 2 - 1);
		println!("siphash keys [0]: {}", u8_to_u64(header_hash, 0));
		Cuckoo {
			v: [
...
impl Miner {
	...
	pub fn mine(&mut self) -> Result<Proof, Error> {
		...
		for nonce in 0..self.easiness {
			...
			println!("Added us[0], vs[0]: {}, {}", us[0], vs[0]);
