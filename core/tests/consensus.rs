...
fn repeat(
	...
	let diffs = vec![Difficulty::from_num(diff.0); len as usize];
	let times = (0..(len as usize)).map(|n| n * interval as usize).rev();
	let pairs = times.zip(diffs.iter());
	pairs
		.map(|(t, d)| Ok((cur_time + t as u64, d.clone(), diff.1)))
		.collect::<Vec<_>>()
}
...
fn add_block(
	...
	// get last interval
	let diff = next_difficulty(1, return_chain.clone()).unwrap();
...
fn next_target_adjustment() {
	...
	assert_eq!(
		next_difficulty(1, vec![Ok((cur_time, Difficulty::one(), None))]).unwrap(),
		(Difficulty::one(), 1),
	);
	assert_eq!(
		next_difficulty(1, vec![Ok((cur_time, Difficulty::one(), Some(10)))]).unwrap(),
		(Difficulty::one(), 1),
	);

	assert_eq!(
		next_difficulty(1, repeat(60, (1, None), DIFFICULTY_ADJUST_WINDOW, None)).unwrap(),
		(Difficulty::one(), 1),
	);
	assert_eq!(
		next_difficulty(1, repeat(60, (1, Some(1)), DIFFICULTY_ADJUST_WINDOW, None)).unwrap(),
		(Difficulty::one(), 1),
	);
	assert_eq!(
	...
	let mut s1 = repeat(60, (500, None), sec, Some(cur_time));
