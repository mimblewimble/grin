...
fn build_tx_kernel() {
	...
	tx.validate(Weighting::AsTransaction, verifier_cache()).unwrap();
...
fn transaction_cut_through() {
	...
	assert!(tx1.validate(Weighting::AsTransaction, verifier_cache()).is_ok());
	assert!(tx2.validate(Weighting::AsTransaction, verifier_cache()).is_ok());
...
fn multi_kernel_transaction_deaggregation() {
	...
	assert!(tx1234.validate(Weighting::AsTransaction, vc.clone()).is_ok());
	...
	assert!(deaggregated_tx34.validate(Weighting::AsTransaction, vc.clone()).is_ok());
	assert_eq!(tx34, deaggregated_tx34);

	let deaggregated_tx12 = deaggregate(tx1234.clone(), vec![tx34.clone()]).unwrap();

	assert!(deaggregated_tx12.validate(Weighting::AsTransaction, vc.clone()).is_ok());
...
fn multi_kernel_transaction_deaggregation_2() {
	...
	assert!(deaggregated_tx3.validate(Weighting::AsTransaction, vc.clone()).is_ok());
...
fn multi_kernel_transaction_deaggregation_3() {
	...
	assert!(deaggregated_tx13.validate(Weighting::AsTransaction, vc.clone()).is_ok());
...
fn multi_kernel_transaction_deaggregation_4() {
	...
	.unwrap();
	assert!(tx12345.validate(Weighting::AsTransaction, vc.clone()).is_ok());
	...
	.unwrap();
	assert!(deaggregated_tx5.validate(Weighting::AsTransaction, vc.clone()).is_ok());
...
fn basic_transaction_deaggregation() {
	...
	assert!(deaggregated_tx1.validate(Weighting::AsTransaction, vc.clone()).is_ok());
	assert_eq!(tx1, deaggregated_tx1);

	let deaggregated_tx2 = deaggregate(tx3.clone(), vec![tx1.clone()]).unwrap();

	assert!(deaggregated_tx2.validate(Weighting::AsTransaction, vc.clone()).is_ok());
...
fn blind_tx() {
	let btx = tx2i1o();
	assert!(btx.validate(Weighting::AsTransaction, verifier_cache()).is_ok());
...
fn tx_build_exchange() {
	...
	tx_final.validate(Weighting::AsTransaction, verifier_cache()).unwrap();
}
