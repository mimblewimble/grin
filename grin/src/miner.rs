...
use pool;
use util;
use wallet::{CbAmount, CbData, WalletReceiveRequest};
...
impl Miner {
			...
			if time::get_time().sec > next_stat_output {
				...
				if let Ok(stat_vec) = stats {
					for s in stat_vec {
						...
						debug!(
							"Mining on Device {} - {}: Last hash time: {} - \
	...
	fn get_coinbase(&self) -> (core::Output, core::TxKernel) {
		if self.config.burn_reward {
			...
			let request = WalletReceiveRequest::Coinbase(CbAmount {
				amount: consensus::REWARD,
			});
