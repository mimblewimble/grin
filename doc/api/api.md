...
This endpoint is used to query a node about various information on the blockchain, networks and peers. By default, this REST API is binded to `localhost:13413`. This API is started as the same time as the Grin node.
...
The wallet owner API is an endpoint to manage the user wallet: broadcast transaction, sign transaction, see the current balance... This REST API can be started with the `grin wallet owner_api` command and is binded to `localhost:13420`. This endpoint must **never** be exposed to the outside world.
