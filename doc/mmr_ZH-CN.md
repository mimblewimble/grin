...
后者依赖于完美二叉树，前者既可以看作是由许多完美二叉树组成的列表或者看作是一个右上角被截断的二叉树。
...
给定一个节点的位置 `n`，我们可以计算它的高度、父母、兄弟姐妹等等。
...
与 Merkle tree 相反，MMR 通常在构造时没有单独的根所以我们需要一种方法来计算一个（不然就违背了使用哈希树本意）。
由于某些原因，此过程称为 “bagging the peaks” 详见 [1] 中描述。

首先，我们确定 MMR 的所有峰；在这里我们将定义这样一种方法。
...
这个 MMR 有 11 个节点，它的所有峰分别位于索引 111（7），1010（10）和 1011（11）。
...
所有这些操作都非常的简单。高度 `h` 下跳至节点的右兄弟节点只需要将 `2^(h+1) - 1` 添加到其位置。
减去 `2^h` 即可跳到他的左孩子节点。
...
当删除掉足够多的叶子后，他们的父亲节点的存在也变得不必要，因此，我们可以通过删除它的叶子来修剪 MMR 的一大部分。
