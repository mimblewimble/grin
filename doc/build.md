...
Grin can also be built in debug mode (without the `--release` flag, but using the `--debug` flag) but this will render fast sync prohibitively slow due to the large overhead of cryptographic operations.
