...
// The role of this thread is to broadcast an aggregated transaction made of
// all the transactions in the stempool every dandelion_patience seconds.
// This is the only way a stem transaction can be broadcasted to the network as
// stem transaction
	...
	let _ = thread::Builder::new()
		...
		.spawn(move || {
			loop {
				...
				let tx_pool = tx_pool.clone();
				let stem_transactions = tx_pool.read().unwrap().stem_transactions.clone();
				let stem_transactions_vec: Vec<Transaction> = stem_transactions
					.iter()
					.map(|(_, tx)| *tx.clone())
					.collect();
				// Take all the stem transaction and make a big transaction
				// The multi kernel stem transaction
				let mk_stem_transaction =
					transaction::aggregate_with_cut_through(stem_transactions_vec);
