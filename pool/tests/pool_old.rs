...
// parent_transaction, false); 		if result.is_err() {
...
// 		// First, add the transaction rooted in the blockchain
// let result = write_pool.add_to_memory_pool(test_source(),
// multi_kernel_transaction, false); 		if result.is_err() {
// 			panic!(
...
// let result = write_pool.add_to_memory_pool(test_source(),
// transaction1.clone(), false); 		if result.is_err() {
...
// 		// First, add the transaction rooted in the blockchain
// let result = write_pool.add_to_memory_pool(test_source(),
// found_tx.clone(), false); 		if result.is_err() {
...
// 			expect_output_parent!(read_pool, Parent::PoolTransaction{tx_ref: _}, 12);
// 		} else {
// expect_output_parent!(read_pool, Parent::StemPoolTransaction{tx_ref: _},
// 12); 		}
// 		expect_output_parent!(read_pool, Parent::AlreadySpent{other_tx: _}, 11, 5);
// 		expect_output_parent!(read_pool, Parent::BlockTransaction, 8);
// 		expect_output_parent!(read_pool, Parent::Unknown, 20);
...
// 		// First, add the transaction rooted in the blockchain
// let result = write_pool.add_to_memory_pool(test_source(),
// parent_transaction, true); 		if result.is_err() {
// 			panic!("got an error adding parent tx: {:?}", result.err().unwrap());
// 		}
//
// 		// Now, add the transaction connected as a child to the first
// let child_result = write_pool.add_to_memory_pool(test_source(),
// child_transaction, false); 		if child_result.is_err() {
// 			panic!(
// 				"got an error adding child tx: {:?}",
// 				child_result.err().unwrap()
// 			);
...
// 	{
// 		let read_pool = pool.read().unwrap();
// // First transaction is a stem transaction. In that case the child
// transaction 		// should be force stem
// 		assert_eq!(read_pool.total_size(), 2);
...
// 						in_chain,
// 						output,
// } => if other_tx.is_some() || !in_chain || output !=
// test_output(7).commitment() 					{
...
// valid_transaction.clone(), false) { 			Ok(_) => {}
// Err(x) => panic!("Unexpected error while adding a valid transaction:
// {:?}", x), 		};
//
...
//
// match write_pool.add_to_memory_pool(test_source(),
// double_spend_transaction, false) { Ok(_) => panic!("Expected error when
// adding double spend, got Ok"), 			Err(x) => {
// 				match x {
...
// // will instead throw a (correct as well) Already spent error. An
// AlreadyInPool // error can only come up in the case of the exact same
...
// false) { Ok(_) => panic!("Expected error when adding already in pool, got
// Ok"), 			Err(x) => {
// 				match x {
