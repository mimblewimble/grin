...
fn test_transaction_pool_block_building() {
	...
	let root_tx_1 = Arc::new(test_transaction(&keychain, vec![10, 20], vec![24]));
	let root_tx_2 = Arc::new(test_transaction(&keychain, vec![30], vec![28]));
	let root_tx_3 = Arc::new(test_transaction(&keychain, vec![40], vec![38]));

	let child_tx_1 = Arc::new(test_transaction(&keychain, vec![24], vec![22]));
	let child_tx_2 = Arc::new(test_transaction(&keychain, vec![38], vec![32]));

	{
		...
		write_pool
			.add_to_pool(test_source(), child_tx_1, false, &header)
			.unwrap();
		write_pool
			.add_to_pool(test_source(), child_tx_2, false, &header)
			.unwrap();
