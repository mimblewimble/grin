...
use types::{BlockChain, Orphans, Parent, Pool, PoolError, TransactionGraphContainer, TxSource};
pub use graph;
	...
	T: BlockChain,
{
	...
	fn search_blockchain_unspents(&self, output_commitment: &Commitment) -> Option<Parent> {
		self.blockchain.get_unspent(output_commitment).map(|_| {
	...
	fn mark_transaction(
		...
		for output in &tx_ref.unwrap().outputs {
			match self.pool.get_internal_spent_output(&output.commitment()) {
				Some(x) => if self.blockchain
					.get_unspent(&x.output_commitment())
					.is_none()
				{
...
mod tests {
	use super::*;
	use types::*;
	use secp::{constants, ContextFlag, Secp256k1};
	use secp::key;
	use core::core::build;
        ...
        ($pool:expr, $expected:pat, $( $output:expr ),+ ) => {
            $(
                match $pool.search_for_best_output(&test_output($output).commitment()) {
                    $expected => {},
                    x => panic!("Unexpected result from output search for {:?}, got {:?}",
						$output,  x),
                };
            )*
        }
	...
	fn test_block_building() {
		...
		let mut txs: Vec<Box<transaction::Transaction>>;
		{
			...
			block =
				block::Block::new(&block::BlockHeader::default(), tx_refs, key::ONE_KEY).unwrap();
		}
