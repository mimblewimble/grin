...
impl TlsApiServer {
	...
	/// under the root URL.
	pub fn new(root: String) -> TlsApiServer {
		TlsApiServer {
			root: root,
			router: Router::new(),
			mount: Mount::new(),
	...
	pub fn start<A: ToSocketAddrs>(&mut self, addr: A) -> Result<(), String> {
		...
		if !Path::new("grin.p12").exists()
		{
			generate_certs("grin.p12".to_string()).unwrap();
		}

		let ssl = NativeTlsServer::new("grin.p12", "").unwrap();
		let result = Iron::new(m).https(addr, ssl);
