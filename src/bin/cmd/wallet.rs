...
use grin_wallet::{
	HTTPWalletToNodeClient, HTTPWalletToWalletClient, LMDBBackend, WalletBackend, WalletConfig, WalletInst, WalletSeed,
};
use keychain;
...
pub fn instantiate_wallet(
	...
	let mut db_wallet =
		LMDBBackend::new(wallet_config.clone(), passphrase, client_n, client_w).unwrap_or_else(|e| {
			panic!(
...
pub fn wallet_command(wallet_args: &ArgMatches, config: GlobalWalletConfig) -> i32 {
	...
	if let ("init", Some(_)) = wallet_args.subcommand() {
		...
		let client_w =
			HTTPWalletToWalletClient::new();
		let _: LMDBBackend<HTTPWalletToNodeClient, HTTPWalletToWalletClient, keychain::ExtKeychain> =
