...
impl<'a> Message<'a> {
	...
	pub fn respond(&mut self, msg_type: Type, body: Vec<u8>) -> Result<(), Error> {
	...
	H: MessageHandler,
{
	...
	thread::Builder::new().name("peer".to_string()).spawn(move || {
		...
		loop {
			// check the read end
			match read_header(conn) {
				Ok(h) => {
					let mut msg = Message::from_header(h, conn);
					if let Some((body, typ)) = try_break!(error_tx, handler.consume(&mut msg)) {
						try_break!(error_tx, msg.respond(typ, body));
					}
				}
				...
				Err(e) => {
					try_break!(error_tx, Err(e));
				}
			}

			// check the write end
			if let Ok(data) = send_rx.try_recv() {
