...
impl KeybaseWalletCommAdapter {
	...
	pub fn new() -> Box<WalletCommAdapter> {
        ...
        proc.arg("keybase").stdout(Stdio::null()).status().expect("Keybase executable not found, make sure it is installed and in your PATH");
...
fn get_unread() -> HashMap<String, String> {
    ...
    for msg in response["result"]["conversations"].as_array().unwrap().iter() {
        if msg["unread"] == true {
...
pub fn receive_tx(host: &str, slate: &Slate) -> Result<Slate, Error> {
    ...
    let mut res = match client.post(&url).json(&slate).send() {
        Ok(r) => r,
        Err(_) => return Err(ErrorKind::WalletComms(format!("Could not connect to {}", url)))?,
    };
    ...
    if !res.status().is_success() {
        return Err(ErrorKind::WalletComms(format!("Status code {} with reason {}",res.status().as_u16(), txt)))?;
    }
                    ...
                    Ok(slate) => {
                        match receive_tx(listen_addr, &slate) {
                            Ok(signed) => {
                                ...
                                if sent { println!("Returned slate to {}", channel); }
                                else { println!("Failed to return slate to {}", channel); }
                            },
