...
fn basic_transaction_api(test_dir: &str) -> Result<(), libwallet::Error> {
	...
	wallet::controller::owner_single_use(wallet1.clone(), |api| {
		let (_, wallet1_info) = api.retrieve_summary_info(true, &wallet1_parent_id)?;
		let (refreshed, txs) = api.retrieve_txs(true, None, &wallet1_parent_id)?;
		assert!(refreshed);
	...
	wallet::controller::owner_single_use(wallet2.clone(), |api| {
		let (refreshed, txs) = api.retrieve_txs(true, None, &wallet2_parent_id)?;
		assert!(refreshed);
		// we should have a transaction entry for this slate
		let tx = txs.iter().find(|t| t.tx_slate_id == Some(slate.id));
		assert!(tx.is_some());
	...
	wallet::controller::owner_single_use(wallet1.clone(), |sender_api| {
		...
		let (_, wallet1_info) = sender_api.retrieve_summary_info(true, &wallet1_parent_id)?;
		// should be mined now
		assert_eq!(
			wallet1_info.total,
...
fn tx_rollback(test_dir: &str) -> Result<(), libwallet::Error> {
	...
	// Check transaction log for wallet 1
	wallet::controller::owner_single_use(wallet1.clone(), |api| {
		let (refreshed, _wallet1_info) = api.retrieve_summary_info(true, &wallet1_parent_id)?;
		assert!(refreshed);
	...
	wallet::controller::owner_single_use(wallet2.clone(), |api| {
		...
		assert_eq!(unconfirmed_count, 1);
		let (refreshed, wallet2_info) = api.retrieve_summary_info(true, &wallet2_parent_id)?;
		assert!(refreshed);
	...
	wallet::controller::owner_single_use(wallet1.clone(), |api| {
		...
		let res = api.cancel_tx(1, &wallet1_parent_id);
		assert!(res.is_err());
		let (_, txs) = api.retrieve_txs(true, None, &wallet1_parent_id)?;
		let tx = txs
			.iter()
		...
		let res = api.cancel_tx(tx.id, &wallet1_parent_id);
		assert!(res.is_err());
	...
	// Wallet 2 rolls back
	wallet::controller::owner_single_use(wallet2.clone(), |api| {
		let (_, txs) = api.retrieve_txs(true, None, &wallet2_parent_id)?;
		let tx = txs
			.iter()
			.find(|t| t.tx_slate_id == Some(slate.id))
			.unwrap();
		api.cancel_tx(tx.id, &wallet2_parent_id)?;
		let (refreshed, wallet2_info) = api.retrieve_summary_info(true, &wallet2_parent_id)?;
		assert!(refreshed);
		// check all eligible inputs should be now be spendable
		assert_eq!(wallet2_info.amount_currently_spendable, 0,);
		...
		// can't roll back again
		let res = api.cancel_tx(tx.id, &wallet2_parent_id);
		assert!(res.is_err());
